...
export default function Portal({ role, email }: { role: string; email: string }) {
  ...
  const handleLoad = useCallback(() => {
    ...
    if (!win || !doc) return
    /* Presentation rules (mobile nav, attendee chips, housekeeping dimming) now live
       inside app.html so they survive every re-render (day switch, view toggle,
       async /api/schedule load). Do not re-add one-shot DOM patching here. */
