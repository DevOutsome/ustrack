...
export default function Portal({ role, email }: { role: string; email: string }) {
  ...
  const handleLoad = useCallback(() => {
    ...
    const fix = doc.createElement('style'); fix.textContent = '.att-chip{min-width:90px;max-width:90px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;justify-content:flex-start;font-size:11.5px!important;padding:4px 8px!important}.att-chip .dot-g,.att-chip .dot-r{flex-shrink:0}.event-card.housekeeping{opacity:.45;border-style:dashed!important;padding:8px 14px!important;min-height:0!important}.event-card.housekeeping .ec-top{padding:0!important;gap:6px!important}.event-card.housekeeping .ec-body{gap:2px!important}.event-card.housekeeping .ec-title{font-size:13px!important}.event-card.housekeeping .ec-meta,.event-card.housekeeping .ec-loc,.event-card.housekeeping .ec-people,.event-card.housekeeping .ec-weather,.event-card.housekeeping .ec-sun,.event-card.housekeeping button,.event-card.housekeeping .ec-going{display:none!important}'; doc.head.appendChild(fix)
